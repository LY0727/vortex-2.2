...
module VX_pending_size #(
    parameter SIZE      = 1,
    parameter INCRW     = 1, // sub read counter
    parameter DECRW     = 1,
    ...
    if (INCRW != 1 || DECRW != 1) begin
        ...
        if (SIZE == 2) begin // what time these two code will enable? cause SIZE was set to 1 in parameter
            assign used_n = used_r ^ (incr ^ decr);
        end else begin
            assign used_n = $signed(used_r) + ADDRW'($signed(2'(incr) - 2'(decr)));
        end

        if (SIZE > 1) begin // what time these two code will enable? cause SIZE was set to 1 in parameter
